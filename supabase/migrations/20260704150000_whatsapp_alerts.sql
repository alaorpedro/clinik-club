-- WhatsApp lead alerts add-on: one private group per funnel, owner-only visibility.
...
-- Read-only for the owning customer. All writes (numbers, status, group_jid, invite_link)
-- go through server functions using the service role, since status is a payment-driven
-- state machine that must never be settable directly by the client.
