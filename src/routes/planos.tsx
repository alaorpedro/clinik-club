      ...
      <main className="flex-1">
        <section className="container mx-auto px-4 py-20">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <div className="ck-r-sig-sm mt-8 inline-flex items-center gap-1 border border-border bg-card p-1">
              <button
                onClick={() => setInterval("monthly")}
                style={{ borderRadius: "var(--ck-r-flat)" }}
                className={`px-5 py-2 text-sm font-semibold transition-colors duration-[180ms] ease-out ${interval === "monthly" ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"}`}
              >
                Mensal
              </button>
              <button
                ...
                style={{ borderRadius: "var(--ck-r-flat)" }}
                className={`px-5 py-2 text-sm font-semibold transition-colors duration-[180ms] ease-out flex items-center gap-2 ${interval === "yearly" ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"}`}
              >
                Anual
                <span className={`text-[10px] px-2 py-0.5 font-bold ${interval === "yearly" ? "bg-highlight text-foreground" : "bg-highlight/20 text-foreground"}`} style={{ borderRadius: "var(--ck-r-flat-sm)" }}>-20%</span>
              </button>
            </div>
          </div>
          <div className="mt-14 grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {plans.map((p) => {
              ...
              return (
              <div key={p.name} className={`ck-card border p-8 ${p.highlight ? "bg-foreground text-background border-foreground shadow-card" : "bg-card border-border shadow-soft"}`}>
                {p.highlight && <span className="inline-block mb-3 bg-highlight text-foreground px-3 py-1 text-xs font-bold" style={{ borderRadius: "var(--ck-r-flat-sm)" }}>Mais popular</span>}
                ...
                <div className="mt-6 flex items-baseline gap-1 flex-wrap">
                  <span className="ck-num shrink-0 whitespace-nowrap text-5xl font-light">{current.price}</span>
                ...
                <Button
                  ...
                  className={`ck-btn mt-6 w-full font-semibold ${p.highlight ? "bg-highlight text-foreground hover:bg-highlight/90" : ""}`}
                >
                  {p.cta}
                </Button>
