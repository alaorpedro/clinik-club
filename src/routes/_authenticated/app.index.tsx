...
function AppHome() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Âncora técnica do CRM (import de contatos) — nunca é um funil de
      // verdade, não mostrar na lista. Ver crmImportFunnelSlug em crm.functions.ts.
      setFunnels((data ?? []).filter((f: any) => !f.slug?.startsWith("crm-import-")));
      try {
  ...
  async function attemptCreateFunnel(name: string, slug: string) {
    try {
      const data = await createFunnelFn({
        data: { name, slug, environment: getPaymentsEnvironment() },
      });
    ...
    <div>
        ...
        <div className="ck-r-sig mb-6 border border-primary/20 bg-primary/5 p-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Você precisa de um plano ativo para criar e publicar funis. Conta criada com sucesso —
              falta só escolher seu plano.
            </p>
          </div>
          <Button
            size="sm"
            onClick={() => setPlansOpen(true)}
            className="ck-btn font-semibold shrink-0"
          >
            Ver planos
          </Button>
        </div>
      ...
      {hasPlan === true && billingIncomplete === true && (
        <div className="ck-r-sig mb-6 border border-[var(--ck-warning)]/30 bg-[var(--ck-warning-bg)] p-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Precisamos do seu CNPJ ou CPF e endereço para emitir a nota fiscal dos seus
              pagamentos.
            </p>
          </div>
      ...
      {usage &&
        (() => {
          ...
          const leadsPct = usage.maxLeadsPerMonth
            ? usage.leadsUsedThisMonth / usage.maxLeadsPerMonth
            : 0;
          ...
          return (
            <div
              className={`ck-r-sig mb-6 border p-5 flex items-center gap-4 ${critical ? "border-destructive/30 bg-destructive/5" : "border-[var(--ck-warning)]/30 bg-[var(--ck-warning-bg)]"}`}
            >
              <div
                className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${critical ? "bg-destructive/15 text-destructive" : "bg-[var(--ck-warning)]/15 text-[var(--ck-warning)]"}`}
              >
                <Crown className="h-5 w-5" />
              </div>
              <div className="flex-1 min-w-0 text-sm">
                <h3 className="font-bold">
                  {critical ? "Limite do plano atingido" : "Você está perto do limite do seu plano"}
                </h3>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {usage.maxFunnels !== null && (
                    <>
                      Funis:{" "}
                      <strong>
                        {usage.funnelsUsed}/{usage.maxFunnels}
                      </strong>
                      .{" "}
                    </>
                  )}
                  Leads este mês:{" "}
                  <strong>
                    {usage.leadsUsedThisMonth.toLocaleString("pt-BR")}/
                    {usage.maxLeadsPerMonth.toLocaleString("pt-BR")}
                  </strong>
                  .
                </p>
              </div>
      ...
      {usage && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
          <Card className="ck-card border-border shadow-soft">
            ...
            <CardContent>
              <div className="ck-num text-2xl font-light">
                {usage.leadsUsedThisMonth.toLocaleString("pt-BR")}
              </div>
              ...
              <div className="mt-3 h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (usage.leadsUsedThisMonth / usage.maxLeadsPerMonth) * 100)}%`,
                  }}
                />
              </div>
            </CardContent>
            ...
            <CardContent>
              ...
              <div className="mt-3 h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${usage.maxFunnels ? Math.min(100, (usage.funnelsUsed / usage.maxFunnels) * 100) : 100}%`,
                  }}
                />
              </div>
            </CardContent>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm md:text-base">
            Crie e gerencie seus funis interativos.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          ...
          {hasPlan === false ? (
            <Button
              onClick={() => setPlansOpen(true)}
              className="ck-btn font-semibold text-xs md:text-sm h-9 md:h-10 px-4 md:px-5"
            >
      ...
      {funnels === null ? (
        ...
        >
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="rounded-2xl border border-border bg-card p-6 shadow-soft animate-pulse"
            >
        ...
        hasPlan === false ? (
          <div className="ck-r-sig border-2 border-dashed border-primary/30 bg-background p-12 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground max-w-md mx-auto">
              Sua conta está pronta. Para criar seu primeiro funil e começar a capturar leads,
              escolha um plano abaixo.
            </p>
            <div className="mt-6 mx-auto max-w-sm space-y-2 text-left text-sm">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-primary shrink-0" />
                <span>Funis ilimitados de captação</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-primary shrink-0" />
          ...
          <div className="ck-r-sig border-2 border-dashed border-border bg-background p-16 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground max-w-sm mx-auto">
              Crie seu primeiro funil interativo e comece a capturar leads em minutos.
            </p>
            <Button onClick={createFunnel} className="ck-btn mt-6 font-semibold">
              <Plus className="h-4 w-4 mr-1" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {funnels.map((f) => (
            <div
              key={f.id}
              className="ck-card border border-border bg-card p-6 shadow-soft hover:shadow-card transition"
            >
              <div className="flex items-center justify-between">
                <h3 className="font-bold">{f.name}</h3>
                <span
                  className={`text-xs px-2 py-0.5 rounded-[var(--ck-r-flat-sm)] ${f.status === "published" ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"}`}
                >
      ...
      {settingsFor && (
        <FunnelSettingsDialog
          ...
          onSlugChange={(slug) =>
            setFunnels(
              (prev) => prev?.map((x) => (x.id === settingsFor ? { ...x, slug } : x)) ?? null,
            )
          }
        />
      )}
